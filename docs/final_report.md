...
| **Report version** | v1 (pre–Week-13 results) — update when portal outputs arrive |
...
## 2. Overall summary (after Week 12)

| Fn | Task | \(d\) | Obs. | Final best \(y\) | Incumbent \(x\) (6 d.p.) | Status |
|----|------|------:|-----:|------------------|---------------------------|--------|
| F1 | Radiation | 2 | 22 | **7.711×10⁻¹⁶** | 0.731024-0.733000 | Unresolved; late signal lobe |
| F2 | Noisy ML | 2 | 22 | **0.776645** | 0.717869-0.020000 | Strong sharp ridge |
| F3 | Drug / adverse | 3 | 26 | **−0.011366** | 0.492581-0.691593-0.401000 | Strong local safe band |
| F4 | Warehouse | 4 | 42 | **0.678600** | 0.404000-0.413000-0.355000-0.413000 | Strong late climb |
| F5 | Chem. yield | 4 | 32 | **3800.74** | 0.440000-0.980000-0.980000-0.980000 | Strong ridge optimum |
| F6 | Cake recipe | 5 | 32 | **−0.136** | 0.441000-0.249000-0.591000-0.729000-0.131000 | Strong W10 basin |
| F7 | HP tuning | 6 | 42 | **1.872233** | 0.073000-0.425000-0.300000-0.157000-0.345000-0.671000 | Strong late climb |
| F8 | 8-param ML | 8 | 52 | **9.872928** | 0.143000-0.061000-0.211000-0.049000-0.413000-0.511000-0.217000-0.916000 | Strong late climb |
...
## 4. Winning-query evidence (incumbents after Week 12)
...
| F5 | 3800.74 | Week 12 | Ridge climb (\(x_1\)=0.44) |
...
**Late evidence.** Weeks 10–12 opened a measurable lobe near `(0.64, 0.68)` with readings −0.00807 → −0.00623 → −0.00512 (still below seed max, but the first usable basin).  
**Status.** Unresolved absolute peak; final round stays in the signal lobe.  
...
**Lesson.** \(x_3\) is sensitive; lock the safe band. (History has 11 weekly points in `data/`.)  
...
**Incumbent.** \(y = 3800.74\) at `[0.44, 0.98, 0.98, 0.98]`.  
**Lesson.** Clearest success: lock high face, climb \(x_1\) (seed ~1089 → ~3801).  
...
## 6. Week 13 / final round (queries locked)
...
**Portal \(y\) for Week 13:** pending — this report’s summary table will be updated when results arrive (`scripts/append_week13.py`).
...
Under a one-query-per-week budget, a **single interpretable GP per function**, specialised with ARD, locks, trust regions and an F1 trust gate, produced clear late gains on F4/F5/F7/F8, a strong ridge on F2, a safe band on F3, and a fragile but real basin on F6. F1 remains the hardest sparse-signal case. Week 13 commits to near-pure exploitation of those validated regions. When portal results return, the overall table and winning-query section of this report will be revised in place — without changing the method narrative.
